...
class Dataset_ETT_hour_decomp(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='ETTh1.csv',
                 target='OT', scale=True, timeenc=0, freq='h'):
        # size [seq_len, label_len, pred_len]
    ...
    def __read_data__(self):
        path = [""] * 3
        path[0] = os.path.join("decomp_data/ETTh1_96_96/train_data.pkl")
        path[1] = os.path.join("decomp_data/ETTh1_96_96/val_data.pkl")
        path[2] = os.path.join("decomp_data/ETTh1_96_96/test_data.pkl")

        path = path[self.set_type]
        data = pickle.load(open(path, 'rb'))

        if self.args.use_trend:
            self.data_x = data['trend_x']
            self.data_y = data['trend_y']
        else:
            self.data_x = data['seasonal_x']
            self.data_y = data['seasonal_y']
        self.data_stamp = torch.zeros_like(self.data_x)
...
class Dataset_TFB(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='Covid-19.csv',
                 target='OT', scale=True, timeenc=0, freq='h'):
        # size [seq_len, label_len, pred_len]
        # info
        self.seq_len = size[0]
    ...
    def __read_data__(self, nrows=None):
        ...
        if data_frame is not None:
            ...
            date = pd.to_datetime(data_frame['date'].values)

            data_stamp = time_features(date, freq=self.freq)
            data_stamp = data_stamp.transpose(1, 0) 

            data_stamp = data_stamp[border1:border2]
        else:
            data_stamp = torch.zeros((self.data_x.shape[0], 4))
...
class Dataset_Wind(Dataset):
    ...
    def __init__(self, args, root_path, flag='train', size=None, features='S', data_path='ETTh1.csv', target='OT', scale=True, timeenc=1, freq='t', seasonal_patterns=None, percent=100):
        # size [seq_len, label_len, pred_len]
        # info
        super().__init__()
        self.seq_len = size[0]
